/*
06) Faça um programa para gerar um arquivo de imagem em escala de cinza (.pgm) 
onde cada linha da imagem tem um valor aleatório entre 0 e 255. A dimensão de 
imagem deve ser de 100 linhas por 100 colunas. 
*/
...
int main() {
    ...
    // Abre (ou cria) o arquivo "imagem_linhas.pgm" no modo de escrita ("w")
    arquivo = fopen("imagem_linhas.pgm", "w");

    // Verifica se o arquivo foi criado/aberto com sucesso
    if (arquivo != NULL) {
        printf("Arquivo criado com sucesso\n");

        // Preenche a matriz 100x100 com números aleatórios entre 0 e 255, um valor por linha
        for (i = 0; i < 100; i++) {
            int valor_linha = rand() % 256; // Valor aleatório entre 0 e 255 para a linha
            for (j = 0; j < 100; j++) {
                matriz[i][j] = valor_linha;
            }
        }

        // Escreve o cabeçalho do arquivo PGM
        fprintf(arquivo, "P2\n"); // Formato do arquivo PGM (texto, grayscale)
        fprintf(arquivo, "#imagem_linhas.pgm\n"); // Comentário com o nome do arquivo
